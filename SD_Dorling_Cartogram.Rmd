...
sdd <- merge( sd.pop, census.data, by.x="GEOID", by.y="tractid" )
...
sdDorling <- merge(sdd_dorling, d0, by.x="GEOID", by.y="tractid2", all.x=T )
```

```{r }

row.ids <- sapply(slot(sdDorling, "polygons"), function(x) slot(x, "ID"))
row.names(sdDorling) <- row.ids

sdDorling <- spTransform(sdDorling, CRS("+proj=longlat +datum=WGS84"))

geojson_write(sdDorling, file="sd_dorling.geojson", geometry="polygon")
```
